...
expect((await app.inject({method:'POST',url:path,headers:{'x-admin-token':'privacy-admin','x-principal-id':'other-user'},payload})).statusCode).toBe(403);
const result=await app.inject({method:'POST',url:path,headers:{'x-admin-token':'privacy-admin','x-principal-id':'support-user'},payload});expect(result.statusCode).toBe(200);
...
const support=await app.inject({method:'POST',url:path,headers:{'x-admin-token':'privacy-admin','x-principal-id':'support-user','idempotency-key':'privacy-plan-support'},payload});expect(support.statusCode).toBe(403);
const headers={'x-admin-token':'privacy-admin','x-principal-id':'lead-user','idempotency-key':'privacy-plan-delete-v2'};
...
const planned=await app.inject({method:'POST',url:`/v1/admin/privacy-requests/${id}/execution-plan`,headers:{'x-admin-token':'privacy-admin','x-principal-id':'lead-user','idempotency-key':'privacy-plan-access-v1'},payload:{expectedVersion:1,reasonCode:'USER_RIGHTS_VERIFIED'}});
...
const planned=await app.inject({method:'POST',url:`/v1/admin/privacy-requests/${created.json().id}/execution-plan`,headers:{'x-admin-token':'privacy-admin','x-principal-id':'lead-user','idempotency-key':'privacy-plan-held-delete-v1'},payload:{expectedVersion:1,reasonCode:'USER_RIGHTS_VERIFIED'}});
