...
it('does not confuse returned stock, channel acceptance and verified refund success',async()=>{const p=await page(true);expect(p.normalize({...record,state:'refund_pending',refund:{submissionState:'accepted'},resolved:false}).label).toBe('退款处理中');expect(p.normalize({...record,resolved:true}).label).toContain('渠道核验成功');expect(p.selectActions({...record,state:'return_received',kind:'return_refund'})).toEqual([]);p.data.capabilities=['commerce.return.inspect'];expect(p.selectActions({...record,state:'return_received'})).toEqual([{action:'inspect_return',label:'记录质检结果'}]);});
it('uses the same key/payload after uncertainty and blocks duplicate submission',async()=>{const p=await page();p.data.basisIndex=6;p.data.note='原始合成说明';let reject!:(e:any)=>void;m.request.mockImplementationOnce(()=>new Promise((_,r)=>reject=r));const write=p.submit();await Promise.resolve();await p.submit();reject({status:504});await write;const original=structuredClone(p.pending);p.data.note='不能替换原请求';m.request.mockResolvedValueOnce(record).mockResolvedValueOnce({items:[record],nextCursor:null}).mockResolvedValueOnce(record);await p.submit();const writes=m.request.mock.calls.map(([x])=>x).filter(x=>x.method==='POST');expect(writes).toHaveLength(2);for(const w of writes){expect(w.idempotencyKey).toBe(original.key);expect(w.data.reason).toBe('原始合成说明');}});
