...
**2026-09-12 当前源码施工状态（不等同上线验收）：** COM-M01/M02 的独立资格、权限、移动管理入口、徽章及稳定码已在隔离数据库验证；读取资料、订单、佣金、改资格、提议和批准费率使用分开的服务端能力。COM-R01 的首次主动确认、一级快照与自荐/二次绑定防线已有测试。COM-R02 的 2000–3500 bps 与双人批准已实现，但管理员费率表单、`expectedVersion` 并发编辑合同和正式生效规则仍未收口。COM-C01 仅有未对外挂载的微信支付 APIv3 协议适配器与内部可靠入站处理，当前结算仍只创建合成待支付订单，不能产生真实收款。COM-C02 的整数分计算、账本投影与支付后计提在隔离测试中有事实边界；真实退款入站、商品级优惠/积分分摊落库、履约解冻、付款预占、转账终态与对账尚未实现，故真实可结算余额保持关闭。COM-C03 仍是待开发及待资金授权状态。COM-U01/U02 的原生创作、私有媒体、审核、公开修订指针和内容安全接入已编码并有隔离测试，公开总开关关闭；真实微信安全回调、COS 权限、原生逐态和真机证据未验证。COM-U03 已有正式流、详情、搜索、点赞/收藏、评论、举报、屏蔽与下架；关注、站内消息和正式申诉案件路径尚未完成。上述未完成项不得从需求表中删除或被解释为已通过。
