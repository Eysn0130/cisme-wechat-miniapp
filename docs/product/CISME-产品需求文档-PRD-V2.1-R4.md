...
Release 0 只交付“购买—护理—复购—自愿分享”的最小生产闭环。外部任务在首发采用邀请制深链，不建设公开任务广场；社区首先是经审的护理故事/阶段证据库，社区交互按已确认后续发布组和治理门禁推进；积分首先是受控激励与抵扣账本；当前 AI 客服只做可手工接管的受控范围，AI 经营平台另期立项。
...
| 产品 R1（已确认后续需求） | 完成运营基础与社区互动 | 安全商品媒体、多 SKU 购物车/收藏、通知；评论/关注/搜索等按治理能力分批启用 | 购物车通过价格/库存/合并/幂等契约；社区通过 UGC 安全、许可、申诉、人员与容量门禁，不再只按旧月份判断 |
...
| 已确认后续发布组 | 安全商品媒体、多 SKU 购物车/收藏、通知、评论/@/关注/作者页/搜索及其治理 | 按 U2–U4 及 CART/UGC 可验收依赖分切片推进；不作空壳、不用旧月份无限延期，也不跳过外部门禁 |
