...
**文档状态：**基线完整性已通过；已记录的 Owner 决策已对齐；法务、财务、资质与其他 TBD 仍须各责任人签字。文件哈希或自动检查通过不等于业务审批。

**发布范围：**公开项目文档

> **一句话目标**
>
> 以同一个原生微信小程序承接“购买—28 天护理—真实阶段证据—复购—自愿分享”的消费者主循环与移动管理操作，以 CISME 自有业务平台持有会员、护理、内容、积分、许可、归因、商品、库存、订单与审计事实。交易已选定自营 `MAKE`；AI 客服先行，AI 经营工作台是达到独立门槛后的后续产品范围；飞书、企业微信或邮件仅作为可选连接器。
...
> **产品结论**
>
> 本项目不是一个聊天机器人，也不是一个孤立商城。它以 One-App 原生微信小程序为用户与移动管理主入口，以 CISME 业务平台作为唯一事实核心。当前先完成护理、客服、内容、积分与自营交易；AI 经营工作台只在后续阶段和权威边界内完成建议、任务、审批、知识、预警与复盘。旧 Web 管理端仅保留为过渡/应急候选，协作软件只是可插拔连接器。

- CISME 业务平台始终是会员、微信身份、护理、内容、积分、投稿、许可、归因与审计的唯一权威数据源。Owner 已选定 `selected_transaction_profile=MAKE`；CISME 同时是商品、订单、库存、支付退款与履约的权威源，不建第二套交易终态。

- 同一小程序中的移动管理页是运营、审核、客服、商品与订单操作的主入口；旧 Web 管理端只作过渡/应急备用，不扩展成第二套产品。AI 对话与飞书、企业微信、邮件都不得成为唯一操作入口。
...
#### 0.1.1 2026-09-12 Owner 决策差异表

| **原口径/位置** | **当前唯一生效口径** | **边界** |
|---|---|---|
| §2.1/§4.4 人工 Web/单一 Web 后台 | One-App：同一原生小程序的移动管理优先；旧 Web 仅过渡/应急 | 不建第二套身份、权限或业务事实 |
| §4.4/原 TBD-22 仍待 MAKE/BUY | 已选自营 `MAKE`，以 [ADR 0005](../adr/0005-commerce-authority-direction.md) 和 [ADR 0009](../adr/0009-pending-payment-order-boundary.md) 承接 | 不重启整套电商 SaaS 选型；微信支付及政策仍按独立门槛 |
| §2.2/§15.1.1 旧月份表可被解读为无限延期 | 购物车、社区交互是已确认的后续产品发布组，按可验收依赖分切片推进 | 不因旧日历无限推迟，也不跳过治理/资金门槛同日全开 |
| §3.3/§9 AI 经营七能力 | 当前优先是人工可接管的 AI 客服；AI 经营平台是独立后续范围 | 不为客服接入预建 OPS-T01–T14/WF01–WF15 或多 Agent 框架 |
| §3.2 “创始人查看全部” | 只能按合法目的、capability、对象/行级范围、字段脱敏和审计查看 | 身份不等于无条件读取 PII，不绕过 maker-checker |
| §8/接口表“签名报价” | 当前 MAKE 协议是服务端持久化、有期限、有所有权校验的 `quote_id`；`request_hash` 是幂等指纹 | 二者都不声称是客户端可验的密码学签名；支付通知验签是独立协议 |
| §15.6 首次体验包前置设备证据 | 开发预览先进入受控体验，再以体验中产出的 iOS/Android/弱网证据决定是否成为待提审候选 | 不用只能在体验版获取的证据阻断首次受控体验 |
| §15.6 私有远端才算 CI | 公开仓库 `Eysn0130/cisme-wechat-miniapp` 已有真实 GitHub Actions 历史；CI 有效性按具体 run/commit/结果证明 | 继续最小 token 权限、secret 隔离、完整 SHA 固定和人工批准发布 |
| 产品 R0/R1/R2 与工程 R1/R2/R3/R4 同名 | 本文用“产品发布组”表示商业范围，用 `U0–U5` 表示当前工程施工切片 | 一个施工切片完成不等于对应产品发布组上线 |
| 旧 REVIEW-STATUS 以哈希作产品审批 | 分开“文件/基线完整性”与“需求决策对齐” | 哈希只能证明比特一致，不替代 Owner/法务/财务/资质签字 |

One-App、自营 MAKE 和新小程序 AppID 已由 Owner 确认，后续实施不再把这三项列为待决策选型；AppID 值与凭证不进入公开 PRD。运费、退款、积分价值、资质、公开许可和费用等未决事项仍保持 TBD，不由实施方补造。
...
| P17 源需求含购物车，但冷启动 SKU 少 | R0 采用独立直接结算；CART 作为已确认 U2/产品 R1 要求推进，购物车不预占、创建订单才预占 | 保留目标能力与契约门禁，不用旧月份无限延期 |
...
| V2.1-R4.1 | 2026-09-12 | Owner 决策对齐 / CISME | 固化 One-App 移动管理优先、自营 MAKE、AI 客服先行；裁决持久化报价协议；修正体验版/CI 循环门禁；分离产品发布组与 U0–U5 工程切片 |
| V2.1-R4.2（施工修订） | 2026-09-12 | 商业社区施工 / CISME | 按本轮 Owner 指令前移普通用户自由图文社区、商业会员管理和单级推荐佣金；新增 §6.8 的稳定需求 ID 与经营审批边界。历史排期文字仅作追溯，不能覆盖本轮已确认业务意图；生产启用仍分别受 UGC、PAY-MAKE、财务与商户资质门禁约束。 |
...
- 用户可查看简明的待审、冻结、可用、已用、失效和欠分。MAKE 只在积分预占/退款契约和财务规则签字后，才可在现金订单按上限抵扣；未通过时只完成积分获取/账本闭环并隐藏消费入口。纯积分和复杂促销不属于 R0。
- 每次受控分享生成唯一 `share_id`，R0 可追踪访问、注册、订单、支付与退款；加购和详细首末触分析按后续阶段启用。
- 退款或异常时自动撤回对应冻结奖励；已解冻积分不足时形成欠分和限制兑换状态，不得静默丢失。
- 同一小程序的移动管理能力具备 R0 所需的用户、护理、任务/审核、积分、商品订单、基础归因、风控、权限、审计、对账和紧急开关；旧 Web 仅作过渡/应急，未启用模块不得以空壳页面计入成功。
...
| **编号** | **成功标准** | **验收方式** |
|---|---|---|
| S1 | 创始人拥有一个统一 AI 入口，同时保留完整的 One-App 人工操作入口 | 使用 10 类真实指令进行路由，并验证 AI 停机时人工流程可继续 |
...
Release 0 的主循环仍是“购买—护理—复购—自愿分享”。2026-09-12 本轮增量施工同时纳入普通用户自由图文社区、商业会员管理和单级推荐佣金；范围纳入不等于资金或公开内容已获生产启用。外部任务仍采用邀请制深链，不能成为站内发帖条件；积分账本与现金佣金账本分开。当前 AI 客服只做可手工接管的受控范围，AI 经营平台另期立项。

| **系统** | **R0 CORE** | **R0 GATED / R1+** | **明确排除** |
|----|----|----|----|
| 微信消费者端 | 微信身份与按需手机号、生命周期首页、D1/D7/D14/D28 护理、邀请制外部投稿、基础分享动作/唯一 share_id、简明积分、商品/直接结算、订单物流售后、协议隐私与许可；本轮新增自由图文草稿、会员身份与稳定推荐码 | 经 UGC 门禁开放本人图文发布、正式信息流、互动和治理；经 PAY-MAKE 与财务门禁开放真实购买后的单级佣金；公开任务广场、个性化推荐、独立消息中心和多 SKU 购物车仍另行裁决 | 站内视频流、复杂 CDP、多级现金分佣、付费加入资格 |
| One-App 移动管理 | 会员、任务/投稿审核、客服、积分账本、商品订单、基础归因、风控、RBAC、审计、对账和紧急开关 | 公共社区治理、复杂促销/成长、算法运营和高级看板按启用能力增加；旧 Web 仅应急 | 无人审核、第二套管理身份/事实、AI 盗图模型、多仓复杂调拨 |
| AI 客服与经营能力 | 人工客服可接管的同库会话、必要审批、结构化报表和确定性告警；AI Provider 未通过门禁时 fail-closed | 客服 AI 通过来源/脱敏/手工接管门禁后先行；OPS-T01–T14、WF01–WF15 和 AI 经营入口是另一产品发布组 | 自动发布、自动改价/投流/采购/退款、为客服预建复杂多 Agent 编排 |
| 集成 | 新 AppID 对应的微信身份、对象存储、物流及自营 MAKE 的支付/退款/库存/积分对账；公开 UGC 时再启用内容安全传播门禁 | 飞书/企微/邮件 Adapter 只有人工复制成本达门槛后选装 | 抓取非公开平台数据、未授权群发营销 |

首发首页按生命周期只突出一个主 CTA：访客看真实证据或购买；首购用户完成今天的护理里程碑；周期完成用户查看总结和补货/复购；符合资格且自愿参与的用户才看到分享任务。积分、社区、商城和邀请可以作为次级入口，但不得在同一状态争夺首屏主行动。

### 2.2 分阶段退出条件

产品路线仍采用证据驱动，但 2026-08 的月份估算不再是生效的延期器。已确认的购物车和社区需求按下列可验收依赖进入后续发布组，不因页面已完成就跳过业务门槛，也不因旧日历无限推迟：

| **产品发布组** | **唯一目标** | **已确认范围** | **可验收依赖/退出证据** |
|----|----|----|----|
| 产品 R0 Core | 建立真实使用与自营交易基线 | One-App 身份/移动管理、生命周期首页、护理里程碑、人工客服、商品/直接结算、订单、邀请制投稿、基础 share_id、隐私合规 | 源码正确性、本地真实集成和受控体验证据；支付/退款/物流未通过则对应能力保持关闭 |
| 产品 R0 Gated | 在已有主循环上安全开放外部依赖 | 真实微信身份、媒体上传、支付/退款/履约、公开 UGC | BASE-WX、PAY-MAKE、UGC-PUBLIC/AIGC-PROP 等各自 `applies_when` 门禁；未通过只关闭相应能力 |
| 产品 R1（已确认后续需求） | 完成运营基础 | 安全商品媒体、多 SKU 购物车/收藏、通知；本轮社区评论/关注/搜索按 §6.8 前移施工 | 购物车通过价格/库存/合并/幂等契约；社区仍通过 UGC 安全、许可、申诉、人员与容量门禁 |
| 产品 R2（后续独立立项） | 用真实 ROI 验证规模化 | 详细归因、有限推荐、高级看板、AI 经营 OPS/WF 与选装连接器 | 内容供需、人工工时、结构化率和 ROI 分别达到已签字进入线 |

当前工程施工使用 `U0–U5` 切片，与上述产品 R0/R1/R2 不同名也不等价：`U0` 为 PRD 对齐与已定位正确性问题；`U1` 为公共布局根因及护理/聊天/商品/结算/编辑代表页；`U2` 为媒体、购物车等运营基础；`U3` 为支付、履约与售后；`U4` 为经门禁的社区与通知；`U5` 为 staging、HTTPS、真实微信身份和 iOS/Android 发布验收。2026-09-12 的实施状态是 `U0 收口、U1 进行中`；该状态只描述工程进度。
...
| R0 生产产品 | 12–16 周基线 | One-App 消费者与移动管理端、后端、账本、自营交易、归因、邀请制投稿与必要治理；旧 Web 仅应急，AI Pilot 不阻塞 | 首发闭环联调、P0/P1 清零；关闭 AI 仍可人工运行 |
...
| **角色** | **职责** | **允许** | **禁止/复核** |
|----|----|----|----|
| 创始人 | 品牌、选题、价格、预算、供应链、重大风险决策 | 按合法目的和已授 capability 查看业务全局脱敏视图；审批关键事项；修改战略规则 | 不因创始人身份越过行级范围、字段级脱敏、PII 最小必要、maker-checker 或审计 |
...
### 3.3 后续 AI 经营总控入口与七类专业能力（非当前客服范围）

当前 AI 范围仅是同库客服会话的受控建议回复与人工接管，不为它建设本节的经营 OPS/WF 或多 Agent 框架。本节只是后续产品 R2 目标：“一个入口”是一个逻辑助理身份和一致会话，不代表绑定某个协作软件。七类专业能力是同一 AI Gateway 下的能力/工具域，不建设七套相互复制数据的 Agent。任何写操作都经 Tool Broker、RBAC、对象版本、金额/影响范围和审批策略校验。
...
下表与紧随其后的图是全生命周期目标架构，不是 R0 部署清单。R0 只部署 One-App 原生小程序（消费者+移动管理）、模块化单体 API、托管 PostgreSQL、对象存储/CDN、同库 outbox/job + Worker、最小状态通知和自营 MAKE；旧 Web 仅作过渡/应急候选。AI Gateway、通用 Notification Hub、连接器和独立数仓分别属于后续/选装范围，不进入 R0 部署拓扑。

| **层级** | **建议组件** | **职责/权威边界** |
|----|----|----|
| One-App 交互 | 原生微信小程序 + CISME 自定义组件 | 消费者护理、社区、任务、积分、商城、消息，以及 capability 控制的移动管理；不持有最终业务事实 |
| 应急操作候选 | 旧 Web 管理端（不扩展） | 过渡期故障处置与人工兜底；共用同一 API/RBAC/审计，不新建身份或业务事实 |
| 领域核心 | 一期采用一个模块化单体 + 托管 PostgreSQL；边界先落实在代码、迁移和权限中，不强制每个领域独立物理 Schema | 会员、护理、活动、投稿、UGC、积分、授权、归因、审计、商品、库存和订单的唯一真相 |
...
flowchart LR
    U["微信用户"] --> MP["原生微信小程序"]
    STAFF["创始人/运营/审核/客服"] --> MP
    STAFF -. "过渡/应急" .-> ADM["旧 Web 管理端"]
    MP --> API["CISME 领域 API"]
    ADM --> API
    API --> DB["PostgreSQL 权威事实"]
    API --> OSS["受控对象存储/CDN"]
    API --> OUTBOX["Outbox / Job / Inbox"]
    MP --> AIGW["AI Gateway + Tool Broker（后续）"]
    AIGW --> KB["版本化知识与证据库"]
    AIGW -- "草稿/审批请求" --> API
    OUTBOX --> HUB["通知中心"]
...
- 任一连接器停用、不可用或迁移时，小程序移动管理仍可完成当前发布组的全部业务；通知与连接器投递不得阻塞原业务事务。
...
| 商品、库存、订单、支付退款与履约 | 已选 `MAKE`：CISME Catalog/Inventory/Order 领域，支付平台只提供经验签/解密/查单/对账后的外部结算事实 | 连接器只读脱敏摘要或提交受控命令；禁止第二套交易终态 |
...
### 4.4 R0 最小生产架构与自营 MAKE 决策

R0 使用公开单仓 `Eysn0130/cisme-wechat-miniapp`：`apps/miniprogram / services/api / services/worker / packages/contracts / infra`；仓库不存放密钥、真实身份或生产数据。部署一个 API 制品和一个 Worker 制品，使用托管 PostgreSQL、私有对象存储/CDN、日志/指标/告警、备份恢复和自营 MAKE 最小集成。公开 UGC 的内容安全/AIGC 传播能力和微信支付 APIv3 都只在各自 `applies_when` 成立时启用。首发不引入第二套管理端/交易系统、经营 AI Gateway、通用 Notification Hub、外部连接器、Kafka、Elasticsearch、独立向量库、数据仓库或通用工作流引擎。

Owner 已完成交易方向决策并唯一写入 `selected_transaction_profile=MAKE`，由 ADR 0005/0009 承接：

- CISME 是商品、订单、库存、支付退款与履约权威源。结算报价由服务端持久化，返回不可枚举的 `quote_id` 和 `expires_at`；下单时重读服务端报价、价格、库存、地址版本和所有权。`request_hash` 仅是幂等请求指纹，不是客户端可验的密码学签名。
- 创建订单时在同库原子预占库存和已启用的积分/一次性券。微信支付 APIv3 的签名验证、回调解密、主动查单、账单对账和发货管理是后续 PAY-MAKE 门禁，不得与报价协议混称。

BUY 仅作历史选项留在修订记录，不进入当前 R0 ERD、OpenAPI、WBS、测试与报价。除非 Owner 以新 ADR 明确重启，不重调研或建设外部电商 SaaS 与双写对账链路。

R0 部署拓扑仅包含：

```mermaid
flowchart LR
    MP["One-App 原生小程序"] --> API["模块化单体 API"]
    LEGACY["旧 Web（过渡/应急）"] -.-> API
...
- `PLANNED → ACTIVE → COMPLETED`；异常分支为 `PAUSED / CANCELLED / TERMINATED`。现金支付成功并进入已结算后只创建 `PLANNED`，不猜测用户已开始使用。
...
23. MAKE 的结算预览是 CISME 服务端持久化、有期限和所有权的报价；创建订单时重读并原子预占库存和已启用的积分/一次性券。

24. MAKE 的微信支付回调验签/解密且幂等，并由主动查单/账单对账收敛；成交奖励先冻结，确认收货并超过售后节点后解冻。

25. 支付失败/取消时，MAKE 幂等释放内部库存、积分及已启用的一次性优惠券。重复回调、定时补偿和人工重放不得重复核销或奖励。
...
29. 创始人在 One-App 移动管理中审批；旧 Web 应急端或选装连接器只可渲染同一审批对象。通过后由中央 Work/Approval 模块生成负责人、截止、完成标准、预算、来源和风险完整的执行任务。
...
| R2 | 个性化推荐、复杂促销/成长和创作者规模化能力 | 通过算法合规、运营产能与 ROI 评审后立项；多 SKU CART 已前移为 U2/产品 R1 |
...
| P17 | 购物车/结算 | R0 直接结算；U2/产品 R1 增加购物车、数量、券与游客合并 | 会员 | 服务端报价，创建订单时才预占库存/积分；CART 不作为 R0 门槛，但不再无日期延期 |
...
社区保持真实护理分享与可信信息的定位。本轮将普通用户无需邀请的纯图、图文、纯文字发布，草稿、审核、正式信息流、必要评论/回复/关注/作者页/搜索和治理前移施工；个性化推荐、陌生人私信、短视频仍不纳入本轮。各项公开能力只有完成 §6.6.2 与 §6.8 的内容安全、许可、人员和技术门禁才可开启，前端实现本身不构成上线理由。
...
R0 的 P17 必须提供可独立深链、可返回的 `CHECKOUT`；在少 SKU 阶段允许从商品详情直接结算。`CART` 是已确认的 U2/产品 R1 后续视图，启用时要求添加、同 SKU 合并、数量修改、选择、删除、失效 SKU、价格/库存变化提示和登录前后合并。MAKE 的购物车/报价阶段不预占库存与积分，只有创建订单时预占。
...
| P15 | `/shop` | products/search；U2 再加 cart summary | R0 搜索/分类并进入商品；U2 增加已确认的加购；实时状态只作展示、下单再校验 | R0 缺货变化；U2 CART-01/02 |
| P16 | `/products/:id` | product/SKU/rules | R0 选 SKU/数量后立即结算；U2 增加加购；库存、抵扣和售后守卫 | R0 价格/库存再校验；U2 CART-03/04 |
| P17 | R0 `/checkout`；U2/产品 R1 `/cart` | MAKE：address/delivery/持久化 quote，已启用时 points；后续 cart/coupon | MAKE 报价/下单/支付且创建订单时预占；购物车阶段不预占 | R0 A07/WX-PAY-MAKE-01；U2 A19/CART-01–08 |
...
| 商品库存 | MAKE 管理 SPU/SKU、价格、条件化积分模式、库存与预警 | 历史订单快照不得随配置变化；所有修改按 capability、对象版本与审计 |
| 订单售后 | 支付、履约、物流、退款、兑换、核销 | MAKE 内部预占同事务并用微信支付事实补偿/对账；订单/支付/履约/售后分轴 |
...
### 6.8 2026-09-12 商业社区与单级推荐增量合同

本节依据 Owner 的本轮施工指令新增稳定 ID；与 §6.2、§6.6.2、§6.7、§7 的隐私、审核和积分事实共同适用。旧章节中“现金佣金不在一期”“评论/搜索以后再做”等排期判断不再阻止本轮开发，但不构成正式经营规则批准或公开/资金启用批准。

| ID | 需求与权威事实 | 施工验收 | 生产启用边界 |
|---|---|---|---|
| COM-M01 | 普通 `member` 账户不自动成为商业会员；资格由授权管理员授予、暂停或到期，保存版本与操作证据 | 普通/会员/管理员正反用例；撤销、到期、切号和越权 | 资格、权益和有效期须由 Owner 批准 |
| COM-M02 | 商业会员有头像角标与稳定不透明推荐码；码可停用，不由 ID/手机号推导 | 小尺寸、长昵称、重复领取、停用和唯一性 | 标识不能误称平台官方认证 |
| COM-R01 | 买家本人显式确认唯一直接推荐关系；普通点击和旧 `share_attribution` 不自动现金化 | A→B→C 仅直接一级；自荐、二次绑定、失效码拒绝 | 归属有效期、复购、旧用户与争议处理待批准 |
| COM-R02 | 默认 2000 bps；会员覆盖 2000–3500 bps；双人审批、版本和生效时间；订单创建时固化规则快照 | 端点、越界、并发、变更前后订单快照 | 修改生效规则及例外由财务/Owner 批准 |
| COM-C01 | 只有验签/解密/查单并核对商户、AppID、订单、金额、币种和身份的支付平台事实可计提；合成待支付订单不产生佣金 | 重复、乱序、取消竞态、对账及零佣金反例 | PAY-MAKE 与支付资质、商户配置和真实协议验证 |
| COM-C02 | 计佣商品现金净额扣除分摊优惠、积分抵扣和累计退款；分为待结算、可结算、处理中、已付及调整/追偿，追加式分录可重建 | 整数分、累计退款、幂等、守恒、并发预占 | 基数、舍入、履约/售后、结算周期、追偿、税务待批准 |
| COM-C03 | 实际转账结果必须由获准渠道核对；受理/等待确认不算已付，未知结果沿原商户单号查询 | 合成协议故障恢复；真实资金动作保持关闭 | 收款人资格、商家转账场景、合同和资金授权另批 |
| COM-U01 | 登录普通用户无需护理邀请即可创建纯图、图文或纯文字；本人草稿、上传素材和版本持久化 | 0/1/9 图、恢复、重试、重排、账号隔离 | UGC_GO_LIVE_GATE + 许可/安全/审核 |
| COM-U02 | 未审的新修订不得覆盖旧公开修订；私有原图经扫描、人工审核和去元数据衍生图后才公开 | 迟到回调、所有权、删除、双人审核、私有/公开媒体隔离 | 微信安全回调配置、审核排班和回滚开关实测 |
| COM-U03 | 公开流、详情、搜索、互动、举报、屏蔽、作者公开资料与必要消息均由正式 UGC 事实驱动 | 正反权限、分页、计数、隐藏/申诉与空错态 | 治理及适用平台规则逐项签字 |

本轮工程默认值仅用于显式合成环境：管理员授予资格；首次显式确认归属；20% 全局费率；订单快照生效；商品级现金净额；累计退款重算。任何环境变量或页面文案都不得把这些默认值等同于已批准的正式经营规则。未批准时现金计提、结算、真实支付与公开社区仍关闭。

**2026-09-12 当前源码施工状态（不等同上线验收）：** COM-M01/M02 的独立资格、权限、移动管理入口、徽章及稳定码已在隔离数据库验证；读取资料、订单、佣金、改资格、提议和批准费率使用分开的服务端能力。COM-R01 的首次主动确认、一级快照与自荐/二次绑定防线已有测试。COM-R02 的 2000–3500 bps 与双人批准已实现，管理员费率两步录入已有原生交互与分位精度校验；`expectedVersion` 并发编辑合同、正式生效规则和原生状态验收仍未收口。COM-C01 仅有未对外挂载的微信支付 APIv3 协议适配器与内部可靠入站处理，当前结算仍只创建合成待支付订单，不能产生真实收款。COM-C02 的整数分计算、账本投影、支付后计提和绑定预存商户退款单的微信验签退款冲销在隔离测试中有事实边界；真实退款申请/查询补偿、商品级优惠/积分分摊的正式政策与落库、履约解冻、付款预占、转账终态与对账尚未实现，故真实可结算余额保持关闭。COM-C03 仍是待开发及待资金授权状态。COM-U01/U02 的原生创作、私有媒体、审核、公开修订指针和内容安全接入已编码并有隔离测试，公开总开关关闭；真实微信安全回调、COS 权限、原生逐态和真机证据未验证。COM-U03 已有正式流、详情、搜索、点赞/收藏、评论、举报、屏蔽与下架；关注关系、关注流与作者公开页已有隔离测试；站内消息和正式申诉案件路径尚未完成，作者页原生状态尚未验收。上述未完成项不得从需求表中删除或被解释为已通过。
...
- R0 若启用积分消费，只允许一种易理解的方式：现金订单在 SKU 上限内使用积分抵扣。MAKE 只在幂等预占、提交、释放、退款分摊和财务规则通过后启用；否则 R0 仅展示积分获取/账本且隐藏消费入口。纯积分实物、积分+现金礼品和券+积分复杂叠加属于后续发布组。
...
本章及 P15–P18、10.3 交易接口和 15.1 交易验收均按已选 `selected_transaction_profile=MAKE` 裁剪：CISME 实现服务端报价、订单、库存账/预占、后续微信支付意图/回调与同库 outbox。BUY 只保留在历史修订记录，不是当前产品、接口或验收分支。

### 8.1 首期商品与结算模式

下表是业务讨论时的首发配置快照，不是写死在软件中的价格或全部 R0 SKU。R0 应先收敛到少量套组/补充装并支持现金支付；只在积分预占/退款契约和财务规则签字后在上限内使用积分抵扣。购物车是已确认后续需求，按 U2 及 CART-01–08 推进；优惠券、纯积分实物和积分+现金礼品仍待各自政策门禁。实际商品、价格、税费、运费、库存与售后政策由后台版本化配置并在上线前签字。
...
本节的服务端报价、三类预占、支付回调和交易实体写入约束全部适用于当前 MAKE。
...
- `checkout_quote` 保存 SKU、数量、金额最小单位、币种、优惠、券、积分规则版本、价格/库存/地址版本、所有者和有效期；客户端只持有 `quote_id`，创建订单时服务端重读并校验。
...
- 实物退款完成不等于可售库存回补；只有退货签收并质检为可售后才写库存回补流水。
- MAKE 每日使用支付平台账单对订单、退款、已启用抵扣积分、奖励积分和库存形成差异单并闭环。
...
- MAKE 下单预占库存，超时未支付触发释放；支付回调与释放任务通过订单版本、预占状态和互斥守卫竞争，最终进入唯一可解释终态。

- 仅当 `points_redemption_enabled=true` 时，MAKE 在提交订单时预占积分；失败/取消释放，唯一结算事实后正式核销，抵扣批次按 FEFO 执行。未启用时不创建积分预占。

- MAKE 订单保存价格、已启用积分比例、优惠券、运费、SKU 和归因快照；后台修改不影响历史。
...
- MAKE 库存不变量：可售、预占、在途等桶均不为负；余额投影等于库存流水聚合；同一订单行最多一个生效预占；任务重放不重复释放或消耗。

## 9. AI 经营工作台目标需求（Pilot / R1 / R2）

本章保留飞书 V1.0 中有价值的后续 AI 经营能力，但不是当前 AI 客服范围，也不是 R0 生产依赖。R0 只建设 One-App 移动管理、必要审批/审计、SQL/模板报表和规则告警；客服 AI 只在来源可追溯、脱敏、草稿不自动发送、人工可接管的边界内先行。只有 2.2 的人工工时、结构化程度和 ROI 门槛通过后，才按本章逐项立项统一经营入口、OPS-T01–T14 与 WF01–WF15。
...
决策卡的权威对象是中央 `approval_request/decision_record`。One-App 移动管理完整展示；旧 Web 应急端或选装连接器可渲染同一对象的摘要和按钮，界面卡片本身不是审批事实。
...
下表是目标能力可能使用的领域实体组，不等同于一表一实体，也不要求 R0 物理创建全部对象。实施方只为当前启用发布组落地必要实体；`follow/recommendation_feedback/coupon/cart`、复杂经营 AI 等后续对象先保留契约与迁移边界，不建空表。已选 MAKE 落地 CISME 交易权威实体。已落地实体的 ERD 与字段字典必须给出字段类型、枚举、唯一键、索引、权限、数据分类、保留期和迁移策略。
...
| 异步集成 | `business_command`、`outbox_event`、`inbox_event`、`scheduled_job`、`integration_delivery`、`connector_command` | 中央命令与投递、消费者去重、租约、死信、人工重驱；交易 outbox 只承载已提交领域事实 |
...
本节是目标接口目录。R0 只实现启用页面、One-App 移动管理闭环和 MAKE 所需契约；`/cart`、优惠券、评论/关注/搜索/推荐、公开主页、详细分享分析、复杂售后、AI/连接器等接口按发布组启用。延期接口不得用始终返回模拟数据的空实现冒充交付；OpenAPI 必须明确 `x-release-stage`、`x-transaction-profile=MAKE`、适用 gate、鉴权、幂等、状态守卫和废弃策略。
...
| GET | `/products`、`/products/{id}` | 商品搜索/列表与商品/SKU 详情 | 列表可短缓存；详情显示售后与条件化抵扣模式；下单时重新校验版本 |
...
| POST | `/v1/me/commerce/quotes` | MAKE 结算报价 | 返回持久化 `quote_id`、`expires_at`、`server_time` 和金额/规则快照；校验所有者、地址、价格、库存和幂等 request_hash，不伪称密码学签名 |
| POST | `/orders` | MAKE 创建订单 | `x-transaction-profile=MAKE`；订单、库存/已启用积分/一次性券预占、outbox 同事务 |
| GET | `/orders`、`/orders/{id}` | MAKE 订单列表/详情 | 本人权限；列表不解密/返回地址，详情才按需最小化读取历史快照 |
...
| POST | `/orders/{id}/cancel` | MAKE 取消订单 | 本人、状态/版本、订单目标与幂等请求一起绑定；释放资源一次 |
| POST | `/orders/{id}/confirm-receipt` | MAKE 确认收货 | 本人、状态与版本校验；不由客户端伪造物流事实 |
| POST | `/orders/{id}/aftersales` | MAKE 发起售后 | 原因、明细、金额、证据与 SLA；审核不直接伪造退款终态 |
| GET | `/orders/{id}/shipments`、`/aftersales/{id}` | MAKE 物流/售后进度 | 本人权限；返回权威事件时间线和下一步 |
...
`business_command` 应加入 10.1 工作/异步实体和字段字典；Adapter 回调、AI Tool Broker、高风险审批和长时重试写意图进入同一命令管线，禁止各自维护“另一套命令事实”。普通 One-App 移动管理/旧 Web 应急端的同步 CRUD 可直接走同一领域 API、对象版本、幂等与审计，不得为形式统一把每次编辑都包装成长流程命令。
...
| 经营决策/红灯 | 创始人/责任人 | 依据、影响、审批与停止线 | One-App 移动管理决策卡；旧 Web 应急或选装连接器只显示摘要/深链 |
...
- 使用公开 monorepo `Eysn0130/cisme-wechat-miniapp`、保护分支、CODEOWNERS、最小 `GITHUB_TOKEN` 权限、secret 隔离、锁文件、许可证清单、SBOM、密钥扫描、依赖审计和数据库迁移测试。仓库公开不授权上传真实个人数据、生产配置或密钥。第三方 GitHub Action 必须审计并固定完整 commit SHA。开发版与体验版上传优先使用微信开发者工具及其内置 CLI；代码上传私钥只允许受保护环境的人工批准任务读取。
...
| CI-05 | `selected_transaction_profile=MAKE` 交易候选 | 持久化报价、库存预占/释放、支付通知验签解密/重放/乱序、主动查单、账单对账与发货契约测试 |
...
| W7–8     | 自营 MAKE 的商品、直接结算、库存、订单/支付/履约、基础售后和对账；积分抵扣另经财务签字 | R0 交易闭环和竞态/事件契约联调；购物车按 U2/CART 后续验收，券/复杂售后仍按门禁 |
...
| **阶段** | **周期** | **范围** | **验收结果** |
|---|---|---|---|
| Manual Core | R0 同步 | One-App 移动管理、Work/Approval、确定性报表/告警、审计；旧 Web 仅应急 | 不依赖模型即可完整运行 |
...
G0 是表中 W1 之前独立的 2 周阶段，退出物包括 PRD/TBD、R0 页面契约与 P01–P23 目标映射、只含 MAKE/已启用能力的 R0 ERD、OpenAPI、事件、WBS 与测试草案、自营 Commerce ADR、当前适用门禁的技术尖峰、隐私与许可文本计划、容量/成本模型、签字 NFR 测量附录和测试策略；未通过不得启动对应工作。
...
下表是目标能力验收目录，不代表全部阻塞 R0。R0 只执行 `R0 CORE` 与实际启用的 `R0 GATED` 用例；A10/A13/A17/A18/A19 中属于公开主页、复杂部分退款、评论、连接器或购物车的场景在对应产品发布组启用时转为门禁；购物车按 U2/产品 R1 执行。资金、积分、身份、许可和安全不变量即使前端尚未开放，也不得被延期功能破坏。
...
| A04    | MAKE 支付权威事实重复入站 3 次 | 验签/解密后的同一支付事实只使订单、预占与奖励更新一次 |
| A05    | 全额退款          | 奖励撤回；抵扣积分原批次退回                 |
| A06    | 奖励已用后退款    | 产生 debt_points 并限制兑换；审计完整        |
| A07    | MAKE 且启用积分抵扣时并发下单 | 库存/积分不超卖、不超扣，失败完整释放，重放不重复效果 |
...
#### 15.1.1 购物车与结算专项验收（CART 为已确认后续发布组）

R0 直接结算按 MAKE 验收：服务端持久化报价必须通过所有权、版本、过期、幂等和重放验收，订单依次通过库存预占、支付、取消、退款和对账；仅在 `points_redemption_enabled=true` 时加测积分预占。CART-01–08 在 U2/购物车发布组启用时成为阻塞门禁，不由旧月份表无限延期。

| **ID** | **场景** | **通过条件** |
|---|---|---|
| CART-01 | 同 SKU 重复加购 | 合并为一行并得到正确数量；重复请求不多加 |
| CART-02 | 数量超过库存/限购 | 服务端拒绝或截断并解释上限，不产生库存预占 |
| CART-03 | 价格或优惠变化 | 购物车标注变化；结算只采用服务端新建且未过期的持久化报价，用户二次确认 |
...
| ADR-01 | 自营 MAKE 决策与边界 | Owner 决策、ADR 0005/0009 和 `selected_transaction_profile=MAKE` 一致；不建 BUY 双写链路；支付、退款、运费、物流和积分价值仍各按未决门禁验收 |
...
只有后续 AI 经营发布组的退出门槛包括：创始人可从统一 AI 入口查询和创建审批请求，One-App 移动管理可完成所有审批/决策；AI 只能读取批准范围并创建草稿/请求；已承诺的任务/复盘/选题能力真实运行；AI 越权和提示注入被阻断；关闭模型或连接器后人工核心流程仍可运行。本段不是当前 AI 客服或 R0 上线前置条件。
...
- 积分价值、有效期、任务规则、保留期、客服、申诉和售后已配置；MAKE 积分抵扣只在预占/退款契约与财务规则签字后配置/验收，未通过则隐藏消费入口。
...
- AI 模型、企业微信、飞书或邮件连接器均不是上线依赖；关闭它们后，One-App 人工管理、审核、交易、任务和审批仍通过核心验收。
...
| 体验版 | 指定体验成员作封闭验收；数据不得混入正式会员资产 | 首次受控体验准入：上传成功、体验名单、真实微信身份、HTTPS 测试 API/合法域名、独立测试数据/存储、能力开关 fail-closed。体验后候选准入：在体验中完成 iOS/Android/弱网/隐私/上传/后台审核证据 | 已提审、已上线、未通过门禁的支付/公众 UGC 可用 |
| 正式版 | 平台审核通过后向目标用户发布 | 15.5 全部适用门槛、BASE-WX、G0-PRE/G0-OPS、法务/隐私、当前交易 profile、灰度/回滚/值守证据通过 | 体验版无故障即可直接发布 |

首个 CISME 受控体验候选以当次不可变 release manifest 为准，不再固定为 2026-08 的旧页面清单。它可包含 One-App 微信身份/移动管理、协议、护理、客服、邀请投稿、人工审核/补件/申诉、简明积分、经审品牌精选、自营商品、直接结算与测试环境的待支付订单/取消。支付 Provider 未通过 PAY-MAKE 时必须明示“暂不支持真实支付”，不伪造已付；没有真实订单/签收源时，只允许凭已批准测试履约证据编号审计式创建体验资格与唯一 PLANNED 护理周期，不得生成收入/支付事实。财务未签字时积分消费关闭；`UGC_GO_LIVE_GATE` 未通过时用户内容公开发布与社区交互关闭；AI 经营工作台、连接器、券和复杂成长不进入首个体验候选。

发布工具不是产品插件。原生运行时继续采用 WXML/WXSS 与微信 API，不引入上传密钥可见的第三方 GitHub Action、非官方打包器、毛玻璃/手势大包或低代码模板。当前公开仓库的 GitHub Actions 已有真实运行历史；每次只能用精确 run URL/ID、commit SHA、job 结果和制品哈希作 CI 证据，YAML 存在本身仍不算通过。微信上传凭证不进入公开仓库或普通 PR job。
...
| DEC-22（原 TBD-22） | V2.1-R4.1 已决 | Owner 已选自营 `MAKE`；CISME Fastify/PostgreSQL/Worker 是交易权威，不建第二套 SaaS；见 ADR 0005/0009 | Owner+产品+技术 | 2026-09-12 已确认；支付/退款/运费/积分政策仍用各自 TBD |
...
|---|---|---|---|---|
| 飞书 | 0 / 0.1 | 项目结论、S1–S6 成功标准 | 0、1.3 | 业务目标保留；删除“飞书唯一入口”，后续经营 AI 必须与 One-App 人工操作并存 |
...
| 小程序 | 10 | P01–P20 页面 | 6.1 | 20/20 保留目标追踪；按 R0 CORE/GATED、R1、R2 启用，P17 R0 直接结算，CART 按 U2/产品 R1 推进；新增 P21–P23 |
...
| **复核对象** | **源数量** | **V2.1 结果** |
|---|---:|---|
| 微信小程序源页面 | P01–P20，共 20 页 | 20/20 有目标追踪；P17 的 CHECKOUT 进入 R0、CART 作为 U2/产品 R1 的多 SKU 后续能力；新增 P21–P23。页面追踪不等于独立路由或首发启用 |
...
| **裁决** | **内容** | **理由/约束** |
|---|---|---|
| R0 CORE | 微信身份/隐私、生命周期首页、D1/D7/D14/D28 护理、邀请制外部投稿、简明积分账/获取闭环、少 SKU 商品/直接结算、订单物流基础售后、许可、One-App 移动管理/RBAC/审计/对账；MAKE 只在契约与财务规则通过后启用现金订单积分抵扣 | 构成“购买—护理—复购—自愿分享”最小闭环；安全、资金和权利边界不缩 |
| R0 GATED | 经审图文、精选/最新 Feed、赞藏、举报/屏蔽、护理记录转公开故事 | 仅 `UGC_GO_LIVE_GATE` 通过后开放；否则只读品牌精选，投稿证据保持私有 |
| 已确认后续发布组 | 安全商品媒体、多 SKU 购物车/收藏、通知；评论/@/关注/作者页/搜索及其治理按 §6.8 前移本轮施工 | 按可验收依赖分切片推进；不作空壳，也不跳过外部门禁 |
| 后续独立立项 | 公开任务广场、个性化推荐、公开六级成长、详细分享分析、券/纯积分/复杂售后、AI 经营 OPS/WF、13 周现金流、连接器 | 分别由业务价值、治理产能、人工工时和 ROI 进入线触发 |
...
| 舍弃 | 注册/完善资料/首次绑定订单独立奖励/每日签到/邀请注册积分；飞书唯一入口、多维表格作数据库、Aily/Workflow 作核心宿主、7 天试点等同生产、七套 Agent、AI 写终态、自动外部发布、非公开抓取、自动改价/投流/采购/退款、积分换好评、医疗诊断、多级佣金、付费取得商业资格、多仓 ERP | 前五类积分制造无效活跃、重复激励或套利；其余属于架构错误、越权合规风险或当前阶段过度建设。单级现金佣金按 §6.8 受门禁增量开发。 |

### I.1 当前原型与生产目标的差距

本文是分阶段生产目标 PRD，不是“当前源码已实现”的证明。当前交付主体是 `apps/miniprogram` 的原生 WXML/WXSS/TypeScript 小程序；历史 React/Vite Web 高保真仅是视觉参考，其构建、路由或截图不能替代小程序验收。R0 保留少 SKU 直接结算；多 SKU CART 是已确认的 U2/产品 R1 需求，按 CART-01–08 推进。
