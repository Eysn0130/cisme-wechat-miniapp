...
- Natural/dynamically measured layout: `[+] [auto-height textarea] [发送]`, 44px minimum targets, 1–5 visual lines, safe-area padding and a distinct focus border. Typed text is never cleared by upload, poll or unrelated failures.
- The attachment sheet offers only `图片`, `拍照`, `选择订单`, and `取消`; 220ms translate/opacity motion with a reduced-motion-compatible nonessential animation.
