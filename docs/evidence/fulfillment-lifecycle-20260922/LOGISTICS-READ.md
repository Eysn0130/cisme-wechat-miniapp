...
发现并修复清空输入仍复用旧凭据，以及请求期间切换操作员后显示旧结果/触发旧导出的风险。请求捕获原API/会话，读取结束复核；编辑会话/环境立即清空旧队列与履约投影、撤销队列请求代次，不保留凭据到存储。5项上下文边界测试及2项真实DOM事件测试通过，覆盖部分队列先返回后切账号、清空凭据后再次加载不发送旧身份。该修复不等于完成正式操作员登录。
