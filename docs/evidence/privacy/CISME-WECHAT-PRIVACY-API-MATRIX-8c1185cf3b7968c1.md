...
| `pages/account`,`pages/settings`；`open-type=chooseAvatar` | `SOURCE`，用户选择的头像，非基础护理必需；§7.7 | 头像；主动点击，拒绝保留默认头像，可重试 | 当前会员；`PUT /v1/me/profile` → `member_profile.avatar_data_url/avatar_revision`；本机派生 JPEG 缓存 `wx.env.USER_DATA_PATH/cisme-avatar-*` | 微信选择器；服务器接收用户选择的图，`POLICY PENDING`；恢复默认头像/权利请求，跨会话清本地缓存；运行时 `BLOCKED` |
