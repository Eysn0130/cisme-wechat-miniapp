...
PR #3 仍为 Draft，隐私执行代码提交 `a80040c`、其后手机号、待支付订单及合成退款授权测试在本轮续建；小程序包源码 SHA-256 `cf5fe2fd9baadf7a71d8bdfb4629b832ddd44559739c2226d2199d402c83c791`。本地隔离回归为 366 单测、562 集成测试（含 386 个签名入口探针），构建、契约和包门禁通过；退款定向集成 17/17 通过。设计结构检查通过但 `releaseReady=false`。224 个注册 `/v1` 方法中严格主体/对象/字段/动作实测 23 个，剩余 201 个未逐项闭环。合成数据本地完整栈及恢复演练完成，但云端独立 staging 未部署。隐私导出仅限测试环境明确资料子集；擦除仅限获二人审批的合成自报微信号，正式导出/删除/注销/撤回均未放行。A 限流和身份源码回归完成，B/C 其余工作仍开放；详见对应分项证据。
...
## Owner 外部门禁解锁包（2026-09-14 现况）

以下“页面”指后台功能区/资源，不假定不同账号的菜单文案完全相同；请在证据中注明实际菜单路径、采集时间和账号/资源掩码。全部先做只读核验，**不需要**密码、Cookie、OTP、AppSecret、私钥、完整证件号或密钥材料。交付截图/状态表前遮盖个人资料和秘密值，但保留可区分资源的掩码及状态。

| 缺什么 | Owner 在哪个后台页面/资源核对 | 最小脱敏状态 | 解锁哪一步 | 只读核验是否有外部副作用 / 后续变更 |
| --- | --- | --- | --- | --- |
| A 微信小程序主体与当前 AppID | 微信公众平台的账号/主体与认证、备案信息 | 主体名称/类型、统一社会信用代码掩码、AppID 掩码、认证有效状态、备案状态/编号掩码 | 与 PRD 已批准的新 AppID 和 B 运营责任主体分别比对，判定法人轴及 AppID 轴 | 无 / 迁移、认证、备案另需批准 |
| B 拟正式运营主体 | Owner 已批准的公司登记/经营与收款责任决定，不从小程序页面文案推断 | 法定名称、信用代码掩码、负责护理/电商/UGC/收款的主体及批准人 | 将 A–E 关系转为可判断的法律/合同关系，而非机械同名 | 无 / 变更运营主体另需决策 |
| C 域名实名、ICP 与服务归属 | 域名注册商实名/解析资源、备案管理系统与实际 API 证书/主机清单 | `cisme.cn` 及 API 子域的实名主体、ICP 状态/编号掩码、证书有效期、承载资源 ID 掩码 | 判定当前合法域名与 API 目标能否用于独立 staging/正式小程序 | 无 / DNS、证书、备案变更另需批准 |
| D 商户与小程序绑定 | 微信支付商户平台的商户资料、AppID 授权绑定、支付能力和交易类发货管理 | mchId 掩码、商户主体、绑定 AppID 掩码、能力/发货管理状态，不提供 APIv3 密钥 | 设计正式支付独立门；不解锁本轮真实资金动作 | 无 / 绑定、支付、退款或转账各需单独批准 |
| E 云账户与私有媒体权属 | 腾讯云账号实名、CAM 访问管理、COS 存储桶列表及桶权限 | 账号主体/租户 ID 掩码、桶名/地域掩码、所有者、公共访问状态及最小可用角色 | 核对媒体隔离与后续部署权限边界 | 无 / 新桶、IAM 授权、公有访问变更另需批准 |
| 类目、资质、接口与隐私声明 | 微信公众平台服务类目/资质、接口设置、用户隐私保护指引 | 类目及审核状态、已批准的数据类别/接口、指引版本与审核状态、功能不符项 | 逐类对照原生隐私矩阵、内部测试包前置条件及正式准入 | 无 / 增类目、改指引或开权限另需批准 |
| 合法域名与版本状态 | 微信公众平台开发设置/服务器域名、版本管理 | request/upload/download/socket 域名、web-view 适用性、开发/体验/审核/正式版号和时间 | 校验当前包网络与版本对应关系；不能用 DevTools 放行域名校验替代 | 无 / 域名修改、上传、提审、发布另需批准 |
| 独立云 staging 目标与授权 | 腾讯云项目/资源清单、API 主机、数据库、worker、COS、日志/队列、配置及 GitHub deployment environment | 资源 ID/地域/租户掩码、与生产逐项隔离、预算/现有资源、只读和部署角色边界、备份/回退路径；不提交秘密值 | 只在固定 HEAD、CI/secret scan 绿、目标与备份/回退均确认后申请远程部署 | 只读无 / 部署、资源增购、网络开放或环境变量写入均另需明确授权 |
| GitHub 合并保护 | 仓库 Settings 的 Rules/Branches 与 Actions checks/deployments | 可用套餐能力、拟保护 `main` 的 check 名称 `verify`、PR/对话解决/up-to-date 可行性、是否有真实 staging deployment status | 提交单人维护可执行的最小 ruleset 方案；不把 skipped 设计/微信 job 当 PASS | 无 / 修改仓库设置另需批准 |
| 获授权设备与测试人员 | 当前 PR3 的 DevTools 工程、微信测试成员/内部测试包、iOS 与 Android 设备 | 设备型号/系统/微信版本、测试人员角色、合成账号类型、可用时间及独立测试 API 目标；无需扫码内容 | 按当前包 SHA 执行关键链和 37 路由适用状态取证 | 只读准备无 / 真实上传、添加体验成员、分发包另需批准 |

执行顺序：先核 A–E 的同日状态，同时分别判断法人和 AppID 是否变化；再核独立云目标与内部测试包前置条件；取得对应授权后分别进行云部署或微信内部测试；最后才是正式验收、ruleset 设置、合并与发布决策。当前这张表没有把未知状态改为 PASS。微信后台会话受浏览器策略阻止、腾讯云控制台停在登录页；不得绕过。无真机时使用 `docs/evidence/visual/CISME-PR3-NATIVE-DEVICE-TEST-SCRIPT-TEMPLATE-2026-09-14.md` 的关键链脚本和空证据模板，不能填虚构结果。PR3 始终 Draft，PR1/PR2/PR3 均不 Merge。

核 COS 权限时参照[腾讯云 COS 访问控制说明](https://cloud.tencent.com/document/product/436/30749)与[桶权限页面说明](https://cloud.tencent.com/document/product/436/30752)；它们只解释应检查的配置，不代表本项目桶已私有。支付绑定前置条件参照[微信支付官方文档](https://pay.wechatpay.cn/doc/v3/merchant/4015459512)，不代表当前商户能力已通过。
