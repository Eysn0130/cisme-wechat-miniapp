...
SOURCE='202653d35b745c9cad2e47cba248dcf9e4706946'
def require(ok,code):
...
def main(run):
    ...
    require(sql('SELECT count(*) FROM schema_migration;')=='75' and sql(invariant)==before,'FORWARD_PRESERVATION_FAILED')
    ...
    report={'runId':run,'sourceHead':SOURCE,'database':database,'postgresVersion':sql('SHOW server_version;'),'baseline':23,'final':75,'injectedJournalFailureRolledBack':True,'syntheticMemberAndConsentDigestPreserved':True,'syntheticRequestPreserved':True,'repeatApplied':0,'productionTouched':False,'oldCismeTestImpact':'UNKNOWN / 未恢复'}
