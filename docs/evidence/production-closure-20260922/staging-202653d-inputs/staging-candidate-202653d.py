...
m.SOURCE='202653d35b745c9cad2e47cba248dcf9e4706946'
m.ARCHIVE_SHA='0e2f04184ef3d21c147833946f56021d90b4873e69ce8d7ddd44c95b5f05fdbf'
def target(hostname,config,old_hash):
    ...
    m.require(db.hostname=='127.0.0.1' and db.path=='/cisme_accept_rc20260922_c8e5a26d' and db.username=='cisme_staging','EXACT_PREVIOUS_ISOLATED_STAGING_REQUIRED')
    m.require(config.get('OBJECT_STORAGE_DRIVER')=='api_gateway','STAGING_LOCAL_STORAGE_REQUIRED')
    m.require(old_hash=='96609889d77612e5ebd5bc487a472b27973879b189a9bf715ffac07469f7bdcd','PREVIOUS_ARTIFACT_DRIFT')
    manifest=json.loads((m.CURRENT/'release-manifest.json').read_text())
    m.require(manifest.get('sourceHead')=='f4c8df9e5be091b246e9893b1c357bc004d943b0','PREVIOUS_SOURCE_DRIFT')
    m.require(str(m.CURRENT.resolve())=='/opt/cisme/releases/rc20260922-c8e5a26d-f4c8df9e5be0','PREVIOUS_PATH_DRIFT')
