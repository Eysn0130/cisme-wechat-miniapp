...
ROOT=pathlib.Path('/opt/cisme');SOURCE='202653d35b745c9cad2e47cba248dcf9e4706946';PREVIOUS='f4c8df9e5be091b246e9893b1c357bc004d943b0'
...
def main(run):
    ...
    require(previous==ROOT/'releases'/'rc20260922-c8e5a26d-f4c8df9e5be0' and manifest['sourceHead']==PREVIOUS,'EXACT_PREVIOUS_RELEASE_REQUIRED')
    require(manifest['hashes'].get('index.js')=='96609889d77612e5ebd5bc487a472b27973879b189a9bf715ffac07469f7bdcd','PREVIOUS_API_DRIFT')
    ...
    require(envfile.read_bytes()==envbytes and hashlib.sha256(obj.read_bytes()).hexdigest()==object_hash and sql('SELECT count(*) FROM schema_migration;')=='75','POST_ROLLBACK_INVARIANT_FAILED')
