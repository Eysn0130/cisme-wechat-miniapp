...
m.SOURCE='44e1789c5998fa21b905bf4a0fca02799ffe3e68'
m.ARCHIVE_SHA='30d737edd47527a1de88fd339cf5df1be945453d01c5f555c940a1762c49ddda'
def target(hostname,config,old_hash):
    ...
    m.require(db.hostname=='127.0.0.1' and db.path=='/cisme_accept_rc20260922_35bce2d8' and db.username=='cisme_staging','EXACT_PREVIOUS_ISOLATED_STAGING_REQUIRED')
    m.require(config.get('OBJECT_STORAGE_DRIVER')=='api_gateway','STAGING_LOCAL_STORAGE_REQUIRED')
    m.require(old_hash=='9e37763d75bf4d189a5dad22de238d56fbffa9ce4863011789d30078cbdae0de','PREVIOUS_ARTIFACT_DRIFT')
    manifest=json.loads((m.CURRENT/'release-manifest.json').read_text())
    m.require(manifest.get('sourceHead')=='f346736d379e9d8b177426f17ffb58a4e4e32cca','PREVIOUS_SOURCE_DRIFT')
    m.require(str(m.CURRENT.resolve())=='/opt/cisme/releases/rc20260922-35bce2d8-f346736d379e','PREVIOUS_PATH_DRIFT')
