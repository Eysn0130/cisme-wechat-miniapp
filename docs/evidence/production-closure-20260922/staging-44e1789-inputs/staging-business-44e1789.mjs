...
try{
 ...
 async function http(path,auth,method='GET',data,idempotencyKey){const response=await fetch('http://127.0.0.1:3100'+path,{method,headers:{...(idempotencyKey?{'idempotency-key':idempotencyKey}:{}),...(auth?{authorization:'Bearer '+auth}:{}),...(data?{'content-type':'application/json'}:{})},body:data?JSON.stringify(data):undefined,signal:AbortSignal.timeout(8000),redirect:'error'});requireFact(response.headers.get('cache-control')==='private, no-store','API_CACHE_BOUNDARY_FAILED');cacheChecks++;const body=await response.json();return{status:response.status,body};}
 ...
 result=await http('/v1/me/aftersales');requireFact(result.status===401,'AFTERSALE_AUTH_REQUIRED');checks.push('aftersale-unauthenticated-denied');
 result=await http('/v1/me/aftersales',ownerToken);requireFact(result.status===200&&result.body.items.length===0,'AFTERSALE_OWNER_EMPTY_FAILED');checks.push('aftersale-owner-empty');
 result=await http('/v1/management/aftersales',operatorToken);requireFact(result.status===403,'AFTERSALE_CAPABILITY_REQUIRED');checks.push('aftersale-operator-denied');
 await pool.query("INSERT INTO authority_grant(member_id,capability,environment,granted_by,grant_reason) VALUES($1,'commerce.aftersale.review','staging',$2,'Synthetic owned aftersale read only')",[operator,principal]);
 result=await http('/v1/management/aftersales',operatorToken);requireFact(result.status===200&&result.body.items.length===0,'AFTERSALE_MANAGER_EMPTY_FAILED');checks.push('aftersale-authorized-queue');
 await pool.query("UPDATE authority_grant SET revoked_at=now(),revoked_by=$2,revoke_reason='Synthetic aftersale revocation' WHERE member_id=$1 AND capability='commerce.aftersale.review' AND revoked_at IS NULL",[operator,principal]);
 result=await http('/v1/management/aftersales',operatorToken);requireFact(result.status===403,'AFTERSALE_REVOKED_CAPABILITY_ACCEPTED');checks.push('aftersale-revoked-queue-denied');
 for(const action of ['constructor','toString','__proto__']){
 result=await http('/v1/management/aftersales/11111111-1111-4111-8111-111111111111/actions',operatorToken,'POST',{action,note:'SYNTHETIC invalid action',expectedVersion:1},'synthetic-invalid-'+action);
 requireFact(result.status===422&&result.body.code==='AFTERSALE_ACTION_INVALID','INHERITED_ACTION_NOT_REJECTED');checks.push('aftersale-invalid-'+action+'-rejected');
 }
