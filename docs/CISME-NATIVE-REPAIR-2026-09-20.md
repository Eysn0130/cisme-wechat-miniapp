...
| PERF-04 | P1 | `services/api/src/operationBudget.ts:44` | NFR-01 / A20 | 真实回环HTTP与合成依赖回归通过；staging待测 | 接收与handler期限分离；ALS单调预算传SQL、fetch、S3/COS；503与未知提交分开 | Fastify真实503/合作取消/请求隔离；COS真实SDK回环取消后零重试请求 | 预算、server、db、storage必须成组回退；不删除鉴权/幂等 |
| PERF-05 | P1 | `services/api/src/db.ts:237` | §13 / A20 | 合成正式单测通过；真实pg故障测试等待CI | 回滚释放后退避；每次尝试一租约，坏连接销毁；40001/40P01有界重试 | 事务13单测；真实pg冲突/终止/回滚/池排队等7测试已编写待执行 | 成组回退事务封装；无schema迁移 |
| PERF-06 | P1 | `services/api/src/db.ts:139` | NFR-01 / A20 | 合成正式单测通过；真实pg故障测试等待CI | 单绝对期限贯穿排队/SQL/退避/提交前；非裸race；COMMIT未知不重试；ROLLBACK命令不称成功 | pool耗尽/查询取消/重试耗尽/丢COMMIT确认/提交tag；待CI真实pg_sleep | 同PERF-05；不能撤销既有合法写入 |
...
Local Node 24.18.0 / Linux: baseline 368 tests; repaired full suite **427/427 in 65 files** passed. Typecheck, package gate, 37-route static audit, design-QA structure, build and contracts have local raw outputs. New seven PostgreSQL fault cases are authored and typechecked but cannot run in this cloud container (no PostgreSQL/Docker); GitHub's explicitly local `cisme_test` CI is the execution target. This document does not predeclare their result. Main/root dependencies unchanged; locked root `npm ci` was run in the source-transfer runner, audit 0 there. A fresh candidate audit is a CI step, not inferred from the old report. Optional `tools/wechat-ci` was not installed or audited anew; its historical 80 upstream alerts remain separately unresolved.
...
| Fastify 5.12.3 / pg 8.23.0 locked source/tests | Reuse; handler cancellation cooperative; pg query_timeout is not physical SQL cancellation; explicit leases tested; no framework replacement |
