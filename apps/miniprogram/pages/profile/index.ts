...
Page({
  data: { memberAvatar: defaultMemberAvatar, chromeStyle: currentChromeStyle(), member: null as any, points: null as any, care: null as any, authority:null as AuthorityProjection|null, commercialEligible:false, supportUnread:0, tasks: [] as any[], tasksLoading: false, tasksError: "", progressPercent: 0, pointsBalanceClass: "", careTitle: "", careCopy: "", careStatus: "", loading: true, navigating: false, loadAttempt: 0, snapshotVersion: 0, tasksAttempt: 0, pageAlive: true, error: "" },
  onLoad() { this.data.pageAlive = true; },
  onResize() { this.setData({ chromeStyle: currentChromeStyle() }); },
  onShow() { if (!requireMemberAccess()) { this.setData({member:null,points:null,care:null,authority:null,commercialEligible:false,supportUnread:0,tasks:[],loading:false}); return; } this.data.pageAlive = true; this.setData({ navigating: false }); const tab = this.getTabBar?.(); if (tab) tab.setData({ active: 3, externalBusy: false }); tab?.syncActive?.(3); const identity=memberIdentity(); if(identity && this.data.member?.id===identity.id)this.setData({member:{...this.data.member,...identity},memberAvatar:identity.avatarUrl}); void this.load(undefined, retainMemberSnapshot(this)); },
  ...
  async load(event?: WechatMiniprogram.TouchEvent, preserveSnapshot = false) {
    ...
    try {
      const [snapshot,authority,support,commercial]=await Promise.all([
        ...
        request<{eligible:boolean}>({path:"/v1/me/commercial-membership",cacheTags:["member"]}).catch(()=>({eligible:false}))
      ]);
      ...
      this.setData({ member:{...safeMember,...memberIdentity()}, memberAvatar:memberIdentity()?.avatarUrl || memberAvatar, points, care: normalizedCare, authority, commercialEligible:commercial.eligible===true, supportUnread:Math.max(0,Number(support.unreadCount)||0), snapshotVersion: snapshot.businessVersion, progressPercent, pointsBalanceClass, careTitle: careView.title, careCopy: careView.copy, careStatus: careView.status, loading: false, error: "" });
    } catch (error) {
      if (this.data.pageAlive && this.data.loadAttempt === attempt) {
        this.data.tasksAttempt += 1;
        this.setData({ member: null, points: null, care: null, authority:null, commercialEligible:false, supportUnread:0, tasks: [], tasksLoading: false, tasksError: "", loading: false, error: "会员资料暂时无法同步，请检查网络后重试。旧积分、护理状态与管理权限不会被当作当前权威。" });
      }
    }
  },
