...
Page({
  ...
  async load(event?: WechatMiniprogram.TouchEvent) {
    ...
    this.setData({ attempt, loading: true, preparing: false, error: "", shareId: "", shareCode: "待生成", history: [] });
    wx.hideShareMenu();
    try {
      const [member, links] = await Promise.all([request<{ display_name: string }>({ path: "/v1/me" }), request<ShareLink[]>({ path: "/v1/me/shares?targetType=invite" })]);
      ...
      this.setData({ displayName: member.display_name || "CISME 会员", shareId: active?.shareId ?? "", shareCode: active ? shortCode(active.shareId) : "待生成", expiresAt: active?.expiresAt ?? "", history: invitations.slice(0, 5).map((link) => ({ ...link, code: shortCode(link.shareId), date: dateLabel(link.createdAt), label: link.state === "active" ? "可分享" : link.state === "expired" ? "已过期" : "已失效" })), loading: false });
      if (active) wx.showShareMenu({ menus: ["shareAppMessage"] });
    } catch {
