...
Page({
 data:{chromeStyle:currentChromeStyle(),authority:null as AuthorityProjection|null,canSupport:false,canCatalog:false,canOrders:false,loading:true,navigating:false,error:""},
 onResize(){this.setData({chromeStyle:currentChromeStyle()});},onShow(){this.setData({navigating:false});void this.load();},
 async load(){this.setData({authority:null,canSupport:false,canCatalog:false,canOrders:false,loading:true,error:""});try{const authority=await authorityProjection();if(!authority.managementAvailable){wx.showToast({title:"当前账号没有管理权限",icon:"none"});wx.navigateBack({fail:()=>wx.switchTab({url:"/pages/profile/index"})});return;}this.setData({authority,canSupport:hasCapability(authority,"support.read"),canCatalog:["commerce.product.manage","commerce.qualification.manage","commerce.inventory.manage"].some((capability)=>hasCapability(authority,capability as any)),canOrders:hasCapability(authority,"commerce.order.read"),loading:false});}catch{this.setData({loading:false,error:"管理权限暂时无法核验，所有管理入口保持关闭。"});}},
