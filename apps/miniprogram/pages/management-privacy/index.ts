...
Page({
 ...
 onStatus(event:WechatMiniprogram.PickerChange){if(this.canAct())this.setData({statusIndex:Number(event.detail.value)===1?1:0});},
 async submit(){
  ...
  try{await request({path:`/v1/management/privacy-requests/${encodeURIComponent(row.id)}/response`,method:"POST",data:{status:this.data.statusIndex===1?"responded":"reviewing",response,expectedVersion:row.version}});
   if(!this.current(epoch,token))return;this.setData({busy:false,selected:null,response:"",notice:"回复已保存。数据导出、删除和注销仍以执行记录为准。"});await this.load();
