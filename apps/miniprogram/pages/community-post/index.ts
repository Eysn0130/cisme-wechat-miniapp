...
Page({
  ...
  async load() {
    ...
    try {
      ...
      this.setData({ post, images: post.media.map((item, index) => ({ id: item.id, index: index + 1,
        src: origin ? `${origin}/v1/ugc/media/${item.id}` : "" })), loading: false });
    } catch (error) { if (epoch === this.data.epoch) this.setData({ loading: false, error: titleOf(error, "护理故事暂时无法加载，请重试。") }); }
  },
  ...
  async sendComment() {
    ...
    const epoch = this.data.epoch, token = getApp<IAppOption>().globalData.sessionToken;
    const body=this.data.comment.trim(),operationKey=this.data.commentKey;
    this.setData({ busy: true, commentAttempted:true, error: "", notice: "" });
    try {
      await request({ path: `/v1/ugc/posts/${this.data.postId}/comments`, method: "POST", idempotencyKey: operationKey,
        data: { body } });
      if (epoch === this.data.epoch && token === getApp<IAppOption>().globalData.sessionToken)
        this.setData({ ...(this.data.comment.trim()===body?{comment:""}:{}), commentKey:key(),commentAttempted:false,
          notice: "评论已提交，审核后会公开。" });
    } catch (error) { if (epoch === this.data.epoch) this.setData({ error: titleOf(error, "评论未提交，请重试。") }); }
  ...
  async report() {
    ...
    try {
      ...
      const category = codes[chosen.tapIndex]; if (!category) return;
      this.setData({ busy: true, error: "" });
      await request({ path: `/v1/ugc/reports/post/${this.data.postId}`, method: "POST", data: { category } });
      this.setData({ notice: "已收到举报，我们会核查这篇内容。" });
    } catch (error) { if (!/cancel/i.test((error as { errMsg?: string })?.errMsg || "")) this.setData({ error: titleOf(error, "举报未提交，请重试。") }); }
    finally { this.setData({ busy: false }); }
  },
  async blockAuthor() {
    if (!this.data.post || this.data.post.isMine || this.data.busy) return;
    if (!getApp<IAppOption>().globalData.sessionToken) { resumeAuthentication(`/pages/community-post/index?id=${this.data.postId}`); return; }
    const answer = await wx.showModal({ title: "屏蔽这位作者？", content: "屏蔽后，你的推荐列表中不再显示其内容。", confirmText: "屏蔽" });
    if (!answer.confirm) return;
    try {
      this.setData({ busy: true, error: "" });
      await request({ path: `/v1/me/ugc/blocks/${this.data.post.authorId}`, method: "PUT", data: { active: true } });
      wx.navigateBack({ fail: () => wx.switchTab({ url: "/pages/community/index" }) });
    } catch (error) { this.setData({ error: titleOf(error, "屏蔽未完成，请重试。") }); }
    finally { this.setData({ busy: false }); }
  },
  editOwn() { if (this.data.post?.isMine) wx.navigateTo({ url: `/pages/community-compose/index?id=${this.data.postId}` }); },
  async deleteOwn() {
    if (!this.data.post?.isMine || this.data.busy) return;
    const answer = await wx.showModal({ title: "删除这篇故事？", content: "删除后不会再公开显示，无法在小程序中恢复。", confirmText: "删除", confirmColor: "#8c354e" });
    if (!answer.confirm || !this.data.post) return;
    try {
      this.setData({ busy: true, error: "" });
      await request({ path: `/v1/me/ugc/posts/${this.data.postId}`, method: "DELETE", data: { expectedVersion: this.data.post.version } });
      wx.navigateBack({ fail: () => wx.switchTab({ url: "/pages/community/index" }) });
    } catch (error) { this.setData({ error: titleOf(error, "删除未完成，请重试。") }); }
    finally { this.setData({ busy: false }); }
  },
