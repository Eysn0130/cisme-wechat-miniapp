...
export function issueSessionToken(payload: Omit<SessionPayload, "expiresAt">, secret: string, now = Date.now()): string {
  const lifetime=payload.scope==="privacy_rights" ? 30*60*1000 : 12*60*60*1000;
  const encoded = Buffer.from(JSON.stringify({ ...payload, expiresAt: now + lifetime })).toString("base64url");
  return `${encoded}.${signature(encoded, secret)}`;
}

export function verifySessionToken(token: string, secret: string, audience: SessionAudience, now = Date.now()): Principal & {scope:"member"|"privacy_rights"} {
  ...
  if (
    ...
    (payload.scope !== undefined && payload.scope !== "member" && payload.scope !== "privacy_rights") ||
    (payload.scope === "privacy_rights" && (payload.adapter !== "wechat" || payload.provider !== "wechat_miniprogram" || payload.expiresAt-now>30*60*1000)) ||
