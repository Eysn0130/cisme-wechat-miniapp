...
正式出站恢复还要求 `COMMERCE_FORMAL_MERCHANT_CERTIFICATE_FILE` 的受保护 PEM：本地精确匹配证书 CN 商户号、证书 serial、私钥公钥及有效期；每次出站再次拒绝过期证书。没有证书绑定仍可按独立回调授权验证历史入站，不会因此封掉回调。该检查不自行信任未知 CA，也不证明证书尚未被平台吊销或 AppID 绑定/产品权限已获批；这些仍须平台事实。合成自签证书只用于测试本地绑定逻辑。
