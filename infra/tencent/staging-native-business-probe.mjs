...
try{
 ...
 const ownerToken=token(owner),operatorToken=token(operator,principal);
 async function http(path,auth,method='GET',data){const response=await fetch('http://127.0.0.1:3100'+path,{method,headers:{...(auth?{authorization:'Bearer '+auth}:{}),...(data?{'content-type':'application/json'}:{})},body:data?JSON.stringify(data):undefined,signal:AbortSignal.timeout(8000),redirect:'error'});const body=await response.json();return{status:response.status,body};}
 ...
 const report={runId:run,instanceId:'lhins-ei4hz4fi',sourceHead:SOURCE,syntheticOnly:true,realWechatLoginVerified:false,realFundsExecuted:false,checks,passed:checks.length,productionTouched:false,fullAcceptanceComplete:false};
